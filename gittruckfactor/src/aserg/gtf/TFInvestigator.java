...
public class TFInvestigator {
	...
	public static void main(String[] args) {
		...
		if (args.length>4)
			chunckSize = Integer.parseInt(args[4]);



		String stdOut;
		try {
			stdOut = createAndExecuteCommand(scriptsPath+"reset_repo.sh "+ repositoryPath + " " + defaultBranch);
...
//			System.out.println(stdOut);
			...
			if (daysBetween(firstCommit.getMainCommitDate(), getLastCommitDate(sortedCommitList))<=chunckSize){
				...
				while (calcDate.getTime().before(getLastCommitDate(sortedCommitList))){
					...
					for (Developer developer : tfDevelopers) {
						...
						if (daysBetween(devLastCommitDate, updated_at)>=chunckSize && devLastCommitDate.before(calcDate.getTime())){
...
//							}
	...
	private static Date getLastCommitDate(List<LogCommitInfo> sortedCommitList) {
