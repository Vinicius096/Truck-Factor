...
public class MigrationProjectFilter extends ProjectFilter {
	...
	public List<ProjectInfo> filter() {
		...
		for (ProjectInfo projectInfo : projects) {
			if (!projectInfo.isFiltered()&&projectInfo.getStatus()!=ProjectStatus.ERROR) {
				try {
					List<LogCommitInfo> sortedCommitList = getProjectSortedCommitList(projectInfo, type);
					List<Float> listNumAddCommitFiles = getListAddsByCommit(sortedCommitList);
	...
	private List<LogCommitInfo> getProjectSortedCommitList(
		...
		stdOut = createAndExecuteCommand(scriptsPath+"get_git_commitlog.sh "+ repositoryPath);
